...
app.directive('inputLabel', function () {
	return {
		...
		template: '<input type="text" class="form-control"  data-ng-model="ngmodel"  placeholder="{{text}}" > ',
		link: function (scope, element, attrs, ngModelCtrl) {
			$(function () {
				var parentElement = $(element).parent();
				console.log(element);
				$(element).focus(function () {
					$(parentElement).prepend($('<label>').html(scope.textElement).addClass('input_label'));
					$(this).css('border-bottom', 'solid #2d6ca2 2px');
				});

				$(element).blur(function () {
					$('.input_label').remove();
					$(this).css('border-bottom', 'solid rgb(195, 192, 192) 1px');
				});
			});
		}
	};
